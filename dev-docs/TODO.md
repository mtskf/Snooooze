...
- [x] High: `Options.jsx` の `import { StorageService } ...` がコンポーネント内に置かれており構文エラーになる。ビルド/実行でクラッシュするため、import をファイル先頭へ移動し未使用 import も整理する必要あり。Options.jsx (line 153)


## Refactoring Opportunities

- [x] Medium: `Options.jsx` の Import/Export ロジック分離。ファイル読み込み、JSONパース、バリデーションロジックがコンポーネント内にあり、肥大化しています。これらを `StorageService.js` 等に移動し、UIは「呼び出すだけ」にシンプル化すべきです。

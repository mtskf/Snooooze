...
- **Alarm System**: `popCheck` runs every minute to restore overdue tabs
- **Storage**: `chrome.storage.local` for snoozed tabs and settings
- **Notifications**: Prompts user when tabs are ready to restore
...
- **Export/Import**: OneTab-compatible format (`URL | Title`)
...
3. Shows notification with "Open Now" / "Postpone" options
4. On click: opens tabs, removes from storage
